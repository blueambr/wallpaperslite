...
<section bind:this={headline}>
	<div class={cx('container', narrowContainer && 'max-w-3xl')}>
		{#if showBackButton}
			...
			></Button>
		{/if}
		<div class={cx('grid gap-4 md:gap-8', !narrowContainer && 'md:grid-cols-2 lg:grid-cols-3')}>
...
<section
	class={cx(
		'fixed top-2.5 left-12 z-10 flex items-center gap-2 rounded-full bg-secondary-dark/50 px-4 py-[11px] opacity-0 shadow-lg  backdrop-blur-md backdrop-saturate-150 transition max-md:duration-500 sm:left-1/2 sm:w-full sm:max-w-fit sm:min-w-60 sm:-translate-x-1/2',
		narrowContainer ? 'left-6 w-[calc(100%-3rem)]' : 'left-12 w-[calc(100%-6rem)]'
	)}
...
>
	...
	{/if}
	<button
		class="group flex grow cursor-pointer items-center justify-center gap-2 p-3 select-none"
		type="button"
